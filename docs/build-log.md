...
- Diagnosed a Gem UI report to literal Markdown escape characters in copied URLs, rather than a Gem API or adapter regression. A literal backslash before the posting ID's underscore is normalized as a path separator by standard URL parsing and therefore cannot identify the intended posting.
- Kept malformed URLs rejected, but now report the actionable non-sensitive correction to paste the browser URL without escaped backslashes. Added single-import and Bulk Add server-action boundary coverage showing a normal Nominal Gem URL reaches the importer unchanged and preserves `source: GEM`; Bulk review cards now label Gem and Rippling sources correctly instead of calling them generic JSON-LD.
...
- Hardened job exports against spreadsheet formula injection with a shared serialization-only cell sanitizer used by both CSV and XLSX generation. It neutralizes formula prefixes after leading whitespace/control characters while preserving numeric export fields and stored application data.
- Added server-side Single Add URL-import limiting through the existing Upstash/in-memory limiter architecture. Single Add now permits 10 imports per authenticated user per minute; Bulk Add retains its separate 3 batch reviews per minute policy so a bulk review does not consume Single Add quota.
